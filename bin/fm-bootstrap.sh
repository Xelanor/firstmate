...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","grok-sub","kimi","cursor","agy","muse","rovo","omp"] | index($h);
    def effort_ok($h; $m; $e):
      if $e == null then true
      ...
      elif $h == "grok" or $h == "grok-sub" then (["low","medium","high"] | index($e))
