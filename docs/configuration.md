...
`grok-sub` uses the already-configured `$HOME/.grok-sub` instead, and the spawn prefixes `GROK_HOME` on the launch so that pool's grok CLI, auth, and hooks share one store.
...
| Custom provider store | Its configured location variables, such as `CODEX_HOME`, `GROK_HOME`, or `XDG_CONFIG_HOME`; Firstmate's existing explicit Claude and Muse store assignments still apply. A `grok-sub` spawn does not depend on this allowlist: it writes `GROK_HOME=$HOME/.grok-sub` inside the launch command itself, so the strip cannot drop the second pool. |
...
`grok-sub` is a valid crewmate/scout profile harness name: it is the grok CLI on the second Grok Heavy home `GROK_HOME=~/.grok-sub`, with the same models and effort values as `grok`.
It is refused for `--secondmate` spawns, like the other crewmate/scout-only adapters.
It is a distinct quota pool, not a new adapter.
`quota-array-dispatch` reads quota-axi for that candidate with `GROK_HOME` set because quota-axi has no grok-sub provider, and scores it in its own `bin/fm-quota-choose.sh` call because one snapshot measures one Grok subscription.
...
GROK_HOME=              # optional Grok config home for firstmate's global grok turn-end hook; defaults to ~/.grok. grok-sub ignores this ambient value and always uses $HOME/.grok-sub
