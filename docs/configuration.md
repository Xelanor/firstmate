...
For grok, `fm-spawn.sh` installs one firstmate-owned global turn-end hook under the per-spawn Grok home (`$GROK_HOME/hooks/`, or `~/.grok/hooks/` when unset) and drops a per-task `.fm-grok-turnend` pointer in the worktree, with teardown removing the task token and pointer.
`grok-sub` uses `$HOME/.grok-sub` unless `--grok-home` names another absolute existing directory, and the spawn prefixes `GROK_HOME` on the launch so the grok CLI, auth, and hooks share that store.
...
| Custom provider store | Its configured location variables, such as `CODEX_HOME`, `GROK_HOME`, or `XDG_CONFIG_HOME`; Firstmate's existing explicit Claude and Muse store assignments still apply. A grok or grok-sub spawn that selected a Grok home also prefixes `GROK_HOME` on the launch command and retains that name when this allowlist is on, so the strip cannot drop the second pool. |
...
The shell scripts do not match those rules; firstmate chooses the best matching rule with judgment, resolves its profile object or array under the operating contract in `AGENTS.md` section 4 and `quota-array-dispatch`, and passes only concrete `--harness`, `--model`, `--effort`, and `--grok-home` flags to `fm-spawn.sh`.
...
`grok-sub` is a valid profile harness name: it is the grok CLI on the second Grok Heavy home `GROK_HOME=~/.grok-sub`, with the same models and effort values as `grok`.
It is a distinct quota pool, not a new adapter.
`quota-array-dispatch` reads quota-axi for that candidate with `GROK_HOME` set because quota-axi has no grok-sub provider.
Do not collapse grok and grok-sub into one Grok, and do not force every Grok candidate onto grok-sub.
`fm-spawn.sh --grok-home` is the per-spawn Grok home; default unset keeps CLI, auth, and turn-end hooks on `~/.grok`.
...
GROK_HOME=              # optional Grok config home for a grok spawn's CLI, auth, and turn-end hook; defaults to ~/.grok. grok-sub ignores this ambient value and uses $HOME/.grok-sub unless --grok-home is passed
