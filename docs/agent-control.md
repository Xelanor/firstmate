...
| `unblock` | Restore steerability of a worker whose input line holds unsubmitted text - the skipped-doorbell condition, where every doorbell ring skips to protect that text and the worker cannot receive messages. Submits the pending text with a verified Enter, then re-rings the steering-inbox doorbell so a skipped steer can land. The agent keeps running; nothing is restarted and no work is discarded. | The stuck text is proven submitted: the composer read `empty` after the Enter. The re-ring that follows reports its own outcome, and only `ring=rang` - the submit verdict read exactly `empty` - proves it landed. `ring=skipped` means the re-rung doorbell is itself sitting unsubmitted and the ladder recorded `skipped-pending`; `ring=unproven` means the submit could not be read either way; `ring=failed` means the keystrokes never reached the pane; `ring=endpoint-gone` means the endpoint died between the submit and the ring; `ring=none` means no unhandled record was left to re-ring. None of those clears the input line or the attempt history. Only the exact `pending` verdict - the one the doorbell ring itself skips on - is typed into: `pending-unproven`, `unknown`, and an unreadable read all refuse without typing anything, and a composer that still holds the text after the Enter budget fails loudly rather than reporting an assumed recovery. An already-empty composer is idempotent success: it re-rings and records that attempt on the ladder, but it never restarts the ladder, because nothing was ever blocked - only a composer proven `pending` and actually unblocked clears the attempt history, and it does so only on `ring=rang`. |
